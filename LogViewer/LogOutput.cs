        ...
        public void processLine(string line)
        {
            try
            {
                ...
                else if (items[0].Contains("GPS") && dflog.logformat.ContainsKey("GPS"))
                {
                    ...
                    if (int.Parse(items[dflog.FindMessageOffset("GPS", "Fix")]) < 3)
                        return;
                    ...
                    double lng = double.Parse(items[dflog.FindMessageOffset("GPS", "Lon")], CultureInfo.InvariantCulture);
                ...
                else if (items[0].Contains("GPOS"))
                {
                    if (dflog.logformat.ContainsKey("GPOS"))
                    {
                        int poslatindex = dflog.FindMessageOffset("GPOS", "Lat");
                        int poslngindex = dflog.FindMessageOffset("GPOS", "Lon");
                        int posaltindex = dflog.FindMessageOffset("GPOS", "Alt");
...
");
            ...
            using (FileStream streamReader = File.OpenRead("C:\\Users\\Tom\\Desktop\\Autel\\sess001\\log001binfp.kml"))
            {
        ...
        public void writeWPFile(string basefilename)
        {
            ...
            foreach (string line in cmdraw)
            {
                ...
                sw.WriteLine("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\t{9}\t{10}\t{11}", CNum, 0, 3, CId, Prm1, Prm2, Prm3, Prm4, Lat, Lng, Alt, 1);
            }
